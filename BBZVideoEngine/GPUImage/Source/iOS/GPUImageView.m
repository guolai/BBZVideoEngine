...
@property (nonatomic, assign) CGRect cacheBounds;
...
//    NSLog(@"Backing width: %d, height: %d", backingWidth, backingHeight);
    ...
    NSAssert(framebufferCreationStatus == GL_FRAMEBUFFER_COMPLETE, @"Failure with display framebuffer generation for display of size: %f, %f", self.cacheBounds.size.width, self.cacheBounds.size.height);
    boundsSizeAtFrameBufferEpoch = self.cacheBounds.size;
...
- (void)recalculateViewGeometry;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if(self.rotationType == BBZGPURotation90 || self.rotationType == BBZGPURotation270) {
...
//    };
    ...
    const GLfloat *retCoord;
    GPUImageRotationMode inputRotation;
    if(self.openMirror) {
        switch (self.rotationType) {
            case BBZGPURotation0:
            {
                retCoord = horizontalFlipTextureCoordinates;
            }
                break;
            case BBZGPURotation90:
            {
                retCoord = rotateRightHorizontalFlipTextureCoordinates;
            }
                break;
            case BBZGPURotation180:
            {
                retCoord = rotate180HorizontalFlipTextureCoordinates;
            }
                break;
            case BBZGPURotation270:
            {
                 retCoord = rotateLeftHorizontalFlipTextureCoordinates;
            }
                break;
            default:
                NSAssert(false, @"rotation error");
                break;
        }
    } else  {
        switch (self.rotationType) {
            case BBZGPURotation0:
            {
                retCoord = noRotationTextureCoordinates;
            }
                break;
            case BBZGPURotation90:
            {
                retCoord = rotateRightTextureCoordinates;
            }
                break;
            case BBZGPURotation180:
            {
                retCoord = rotate180TextureCoordinates;
            }
                break;
            case BBZGPURotation270:
            {
                retCoord = rotateLeftTextureCoordinates;
            }
                break;
            default:
                NSAssert(false, @"rotation error");
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        glVertexAttribPointer(displayTextureCoordinateAttribute, 2, GL_FLOAT, 0, 0, [self textureCoordinatesForRotation]);
