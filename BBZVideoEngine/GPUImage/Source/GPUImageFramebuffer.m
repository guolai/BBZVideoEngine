...
@interface GPUImageFBWeakObject : NSObject
...
@implementation GPUImageFBWeakObject

@end

@interface GPUImageFramebufferManager ()
@property (nonatomic, assign) NSUInteger lastIndex;
@end

@implementation GPUImageFramebufferManager
+ (GPUImageFramebufferManager *)shareInstance
{
    static GPUImageFramebufferManager *fb = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        fb = [[GPUImageFramebufferManager alloc] init];
    });
    return fb;
}
...
- (instancetype)init
{
    if(self = [super init])
    {
        _fbWeakObjectArray = [NSMutableArray arrayWithCapacity:10];
        _fb_queue = dispatch_queue_create("com.hao.ye.GPUImageFramebufferManager", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
- (void)printAllLiveObject {
    ...
    dispatch_async(_fb_queue, ^{
        ...
        for (GPUImageFBWeakObject *weakObject in self.fbWeakObjectArray) {
            ...
            {
                GPUImageFramebuffer *fb = (GPUImageFramebuffer *)weakObject.object;
                if(bShouldLog)
                {
                    [string appendString:NSStringFromCGSize(fb.size)];
                    [string appendString:@"=="];
...
- (void)addGPUFrameBuffer:(GPUImageFramebuffer *)fb
{
#ifdef DEBUG
    dispatch_async(_fb_queue, ^{
        GPUImageFBWeakObject *object = [[GPUImageFBWeakObject alloc] init];
        object.object = fb;
        [self.fbWeakObjectArray addObject:object];
    });
#endif
}
...
//- (NSInteger)framebufferReferenceCount;
...
#ifdef DEBUG
//    NSLog(@"GPUImageFramebuffera  alloc %p",self);
    [[GPUImageFramebufferManager shareInstance] addGPUFrameBuffer:self];
#endif
...
- (id)initWithSize:(CGSize)framebufferSize overriddenTexture:(GLuint)inputTexture renderTexture:(CVOpenGLESTextureRef)renderTexture
{
    if (self = [self initWithSize:framebufferSize overriddenTexture:inputTexture])
    {
...
{
#ifdef DEBUG
//        NSLog(@"GPUImageFramebuffera  dealloc %p",self);
#endif
...
- (void)generateFramebuffer;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([GPUImageContext supportsFastTextureUpload])
        {
            ...
            if(maxIndex < _texture)
            {
                maxIndex  = _texture;
            }
#ifdef DEBUG
            NSLog(@"GPUImageFramebuffer  texttureindex:%d", _texture);
#endif
